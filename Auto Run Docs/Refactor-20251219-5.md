...
- [x] **Task 87:** `src/renderer/hooks/useAgentErrorRecovery.tsx`
  ...
  **Audit Result:** ⚠️ Recovery actions fixed + tests added. Wired `onRestartAgent` into an explicit "Restart Agent" action for `agent_crashed`, and replaced the no-op "Use Terminal" action with a real `onAuthenticate` callback (with Claude-specific labeling). Updated App to switch to terminal mode on auth recovery and focused input. Added hook tests covering auth, crash, and rate-limit actions.

- [x] **Task 88:** `src/renderer/hooks/useAgentExecution.ts`
  ...
  **Audit Result:** ✅ Usage accumulation deduped + typing tightened. Consolidated usage stats aggregation into a shared helper to avoid duplicate logic across batch/synopsis handlers, and corrected `spawnBackgroundSynopsisRef` to reflect its nullable ref behavior. Added hook tests covering batch spawn aggregation, queued item handling, synopsis resume behavior, and flash notification auto-dismissal.

- [x] **Task 89:** `src/renderer/hooks/useAgentSessionManagement.ts`
  ...
  **Audit Result:** ⚠️ Dead paths + metadata handling tightened. Removed unused `startNewAgentSession` API (and App wiring) since it was never referenced, preventing no-op exports. Added cross-session guard so history entries only attach `contextUsage` when writing for the active session. Improved resume metadata lookup to only query origins when session name/starred values are missing, while preserving provided values. Added a dedicated hook test suite covering history writes, cross-session overrides, resume switching, and metadata loading behavior.

- [x] **Task 90:** `src/renderer/hooks/useAutoRunHandlers.ts`
  ...
  **Audit Result:** ⚠️ Robustness + typing cleanup. Removed debug console logging from `handleStartBatchRun`, tightened `AutoRunTreeNode` to a recursive type, and made `handleAutoRunFolderSelected` clear document lists + selected file on list failures (with a defensive try/catch). Also ensured `handleAutoRunCreateDocument` refreshes the document tree alongside the list. Updated tests to cover list/tree clearing and tree refresh.

- [x] **Task 91:** `src/renderer/hooks/useAutoRunImageHandling.ts`
  ...
  **Audit Result:** ⚠️ Robustness + duplication cleanup. Removed debug console logging from paste handling, added stale-guarded attachment loading that clears previews on document changes and failure, and unified markdown cleanup for encoded/raw attachment paths so lightbox deletes match the remove-attachment behavior. Added a test to cover raw-path lightbox deletion.

- [x] **Task 92:** `src/renderer/hooks/useAutoRunUndo.ts`
  ...
  **Audit Result:** ⚠️ Fixed baseline undo snapshots. `pushUndoState` previously skipped snapshots when content equaled `lastUndoSnapshotRef`, which prevented the first edit after a reset from being undoable. Updated the guard to skip only when the last undo entry already matches the snapshot, allowing initial baseline snapshots while still preventing duplicates. Adjusted the corresponding test and kept debounce/stack behavior intact.

- [x] **Task 93:** `src/renderer/hooks/useFileExplorer.ts`
  ...
  **Audit Result:** ⚠️ Dead hook removed + FileNode centralized. `useFileExplorer` was unused in renderer code (only referenced by its own tests and type imports). Removed the hook and its test suite, moved `FileNode` to `src/renderer/types/fileTree.ts`, and updated all imports (FilePreview, MarkdownRenderer, TerminalOutput, remarkFileLinks, tab/mention completion, FileExplorerPanel, FileSearchModal, and related tests). Updated `ARCHITECTURE.md` to document `useFileTreeManagement` instead. Targeted tests pass (useAtMentionCompletion, useTabCompletion, remarkFileLinks).

- [x] **Task 94:** `src/renderer/hooks/useFileTreeManagement.ts`
  ...
  **Audit Result:** ⚠️ Typing + duplication cleanup. Removed the duplicate `RightPanelHandle` interface in the hook and re-exported the canonical type from `components/RightPanel`. Tightened `filteredFileTree` to `FileNode[]` and avoided `any` in the filter reducer. Added a focused hook test suite covering refresh success/error, git+history refresh, fuzzy filtering, and initial tree load (5 tests).

- [x] **Task 95:** `src/renderer/hooks/useGitStatusPolling.ts`
  ...
  **Audit Result:** ⚠️ Polling option + stale state fix. `startPolling` ignored `pauseWhenHidden=false` and still refused to poll when the document was hidden; fixed the guard to respect the option and ensured activity-based restarts honor it too. Also cleared stale `gitStatusMap` when no git repos remain so old data isn’t retained. Added a focused hook test suite covering map clearing and hidden-document polling.

- [x] **Task 96:** `src/renderer/hooks/useGroupManagement.ts`
  ...
  **Audit Result:** ⚠️ Dead modal state removed + create flow simplified. Trimmed inline rename values and ignored empty names to match modal behavior, removed unused create-group modal state/handlers from the hook, and simplified Quick Actions/CreateGroupModal to drop unused "move session to new group" wiring. Updated CreateGroupModal to use `generateId()` for group IDs and added a focused `useGroupManagement` hook test suite. QuickActions/CreateGroupModal tests updated accordingly.

- [x] **Task 97:** `src/renderer/hooks/useInputProcessing.ts`
  ...
  **Audit Result:** ⚠️ Type mismatch + cleanup. Fixed several issues:
  1. **Type mismatch**: `BatchState` interface was a simplified subset that didn't match actual usage (code accessed `worktreeActive` not on the interface). Removed the duplicate interface and made `BatchState` a type alias of `BatchRunState` from `../types` with a deprecation notice.
  2. **Dead dependency**: `activeBatchRunState` was destructured and in useCallback deps array but never actually used in the function body - removed from both.
  3. **Debug logging**: Removed 3 `console.log` calls that were left from development (lines 127-134, 136, and 397-399).

  Core functionality is sound - properly handles slash commands, message queuing, terminal cd tracking, batch mode spawning, and web broadcasting. Build succeeds, useBatchProcessor tests pass (25 tests).

- [x] **Task 98:** `src/renderer/hooks/useKeyboardNavigation.ts`
  ...
  **Audit Result:** ⚠️ Dead hook removed + null safety fix + tests added. Found that `useFocusManager.ts` was never used by application code (only by its own tests) and defined a duplicate `FocusArea` type with 'modal' that differed from `types/index.ts`. Removed the dead hook and its 564-line test file. Also removed unused `editingSessionId` and `editingGroupId` deps from `useKeyboardNavigation` interface (they're checked in `useMainKeyboardHandler.ts` before handlers are called). Fixed null target handling in keyboard event handlers by adding optional chaining (e.target can be null in synthetic events). Added comprehensive test suite (22 tests) covering sidebar navigation, Tab panel switching, Enter activation, and Escape handling. Build succeeds.

- [x] **Task 99:** `src/renderer/hooks/useMainKeyboardHandler.ts`
  ...
  **Audit Result:** ✅ Clean. All exports (`keyboardHandlerRef`, `showSessionJumpNumbers`) are actively used in App.tsx and SessionList.tsx. Hook uses modern React patterns: refs for performance (empty deps array avoids re-attaching listener), useCallback for memoization. `KeyboardHandlerContext = any` is intentional and well-documented (avoids circular deps with App.tsx internals). The inline pass-through shortcut checks (isCycleShortcut, isLayoutShortcut, etc.) are not duplication - they run BEFORE context ref is accessed for modal/layer bypass logic. Architecture is clean: useKeyboardShortcutHelpers for matching, useKeyboardNavigation for navigation handlers, useMainKeyboardHandler for event listener setup. No tests existed; added comprehensive test suite (18 tests) covering hook initialization, browser refresh blocking, session jump number badge state, modal/layer interaction, navigation handler delegation, and session jump shortcuts. Build succeeds.
