...
contextBridge.exposeInMainWorld('maestro', {
	...
	process: {
		...
		onSessionId: (callback: (sessionId: string, agentSessionId: string) => void) => {
			const handler = (_: any, sessionId: string, agentSessionId: string) =>
		...
		onSlashCommands: (callback: (sessionId: string, slashCommands: string[]) => void) => {
			const handler = (_: any, sessionId: string, slashCommands: string[]) =>
		...
		onRemoteNewTab: (callback: (sessionId: string, responseChannel: string) => void) => {
			const handler = (_: any, sessionId: string, responseChannel: string) =>
		...
		onRemoteRenameTab: (callback: (sessionId: string, tabId: string, newName: string) => void) => {
			const handler = (_: any, sessionId: string, tabId: string, newName: string) =>
		...
		onUsage: (
			...
			const handler = (_: any, sessionId: string, usageStats: any) =>
	...
	context: {
		...
		cancelGrooming: () => ipcRenderer.invoke('context:cancelGrooming') as Promise<void>,
	...
	git: {
		...
		onWorktreeDiscovered: (
			...
			const handler = (
				...
				data: { sessionId: string; worktree: { path: string; name: string; branch: string | null } }
			) => callback(data);
	...
	fs: {
		...
		rename: (oldPath: string, newPath: string, sshRemoteId?: string) =>
			ipcRenderer.invoke('fs:rename', oldPath, newPath, sshRemoteId) as Promise<{
				success: boolean;
			}>,
		...
		countItems: (dirPath: string, sshRemoteId?: string) =>
			ipcRenderer.invoke('fs:countItems', dirPath, sshRemoteId) as Promise<{
				fileCount: number;
				folderCount: number;
			}>,
	},
	...
	agents: {
		...
		refresh: (agentId?: string, sshRemoteId?: string) =>
		...
		getCustomPath: (agentId: string) => ipcRenderer.invoke('agents:getCustomPath', agentId),
		...
		getCustomEnvVars: (agentId: string) =>
			ipcRenderer.invoke('agents:getCustomEnvVars', agentId) as Promise<Record<
				string,
				string
			> | null>,
		getAllCustomEnvVars: () =>
			ipcRenderer.invoke('agents:getAllCustomEnvVars') as Promise<
		...
		discoverSlashCommands: (agentId: string, cwd: string, customPath?: string) =>
			ipcRenderer.invoke('agents:discoverSlashCommands', agentId, cwd, customPath) as Promise<
				string[] | null
			>,
	},
	...
	sshRemote: {
		...
		getSshConfigHosts: () =>
			ipcRenderer.invoke('ssh-remote:getSshConfigHosts') as Promise<{
				success: boolean;
				hosts: Array<{
					host: string;
					hostName?: string;
	...
	sync: {
		...
		getSettings: () =>
			ipcRenderer.invoke('sync:getSettings') as Promise<{
				customSyncPath?: string;
			}>,
		getCurrentStoragePath: () =>
			ipcRenderer.invoke('sync:getCurrentStoragePath') as Promise<string>,
		selectSyncFolder: () => ipcRenderer.invoke('sync:selectSyncFolder') as Promise<string | null>,
		setCustomPath: (customPath: string | null) =>
			ipcRenderer.invoke('sync:setCustomPath', customPath) as Promise<{
				success: boolean;
				migrated?: number;
				errors?: string[];
	...
	power: {
		setEnabled: (enabled: boolean) =>
			ipcRenderer.invoke('power:setEnabled', enabled) as Promise<void>,
		isEnabled: () => ipcRenderer.invoke('power:isEnabled') as Promise<boolean>,
		getStatus: () =>
			ipcRenderer.invoke('power:getStatus') as Promise<{
				enabled: boolean;
				blocking: boolean;
				reasons: string[];
		...
		removeReason: (reason: string) =>
			ipcRenderer.invoke('power:removeReason', reason) as Promise<void>,
	},

	// Updates API
	updates: {
		check: (includePrerelease?: boolean) =>
			ipcRenderer.invoke('updates:check', includePrerelease) as Promise<{
		...
		download: () =>
			ipcRenderer.invoke('updates:download') as Promise<{ success: boolean; error?: string }>,
		install: () => ipcRenderer.invoke('updates:install') as Promise<void>,
		getStatus: () =>
			ipcRenderer.invoke('updates:getStatus') as Promise<{
				status:
					| 'idle'
					| 'checking'
					| 'available'
		...
		setAllowPrerelease: (allow: boolean) =>
			ipcRenderer.invoke('updates:setAllowPrerelease', allow) as Promise<void>,
	},
	...
	logger: {
		...
		isFileLoggingEnabled: () =>
			ipcRenderer.invoke('logger:isFileLoggingEnabled') as Promise<boolean>,
	...
	claude: {
		...
		getSessionTimestamps: (projectPath: string) => {
			...
			return ipcRenderer.invoke('claude:getSessionTimestamps', projectPath) as Promise<{
				timestamps: string[];
			}>;
		},
		...
		getAllNamedSessions: () => {
			...
			return ipcRenderer.invoke('claude:getAllNamedSessions') as Promise<
				Array<{
					agentSessionId: string;
					projectPath: string;
					sessionName: string;
	...
	agentSessions: {
		...
		getAllNamedSessions: () =>
			ipcRenderer.invoke('agentSessions:getAllNamedSessions') as Promise<
				Array<{
					agentId: string;
					agentSessionId: string;
					projectPath: string;
		...
		getOrigins: (agentId: string, projectPath: string) =>
			ipcRenderer.invoke('agentSessions:getOrigins', agentId, projectPath) as Promise<
	...
	notification: {
		...
		stopSpeak: (ttsId: number) => ipcRenderer.invoke('notification:stopSpeak', ttsId),
	...
	attachments: {
		...
		list: (sessionId: string) => ipcRenderer.invoke('attachments:list', sessionId),
		getPath: (sessionId: string) => ipcRenderer.invoke('attachments:getPath', sessionId),
	},
	...
	groupChat: {
		...
		resetParticipantContext: (id: string, name: string, cwd?: string) =>
			ipcRenderer.invoke('groupChat:resetParticipantContext', id, name, cwd) as Promise<{
		...
		onParticipantsChanged: (
			...
			const handler = (_: any, groupChatId: string, participants: any[]) =>
	...
	stats: {
		...
		getDatabaseSize: () => ipcRenderer.invoke('stats:get-database-size') as Promise<number>,
	...
	leaderboard: {
		getInstallationId: () =>
			ipcRenderer.invoke('leaderboard:getInstallationId') as Promise<string | null>,
		submit: (data: {
			email: string;
			displayName: string;
		...
		get: (options?: { limit?: number }) => ipcRenderer.invoke('leaderboard:get', options),
...
export interface MaestroAPI {
	...
	agents: {
		...
		refresh: (
			agentId?: string,
			sshRemoteId?: string
		) => Promise<{ agents: AgentConfig[]; debugInfo: any }>;
