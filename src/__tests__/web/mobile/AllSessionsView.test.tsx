...
describe('AllSessionsView', () => {
  ...
  describe('MobileSessionCard', () => {
    ...
    describe('active session styling', () => {
      it('applies active styling when session is active', () => {
        ...
        render(<AllSessionsView {...createDefaultProps({ sessions, activeSessionId: 'session-1' })} />);
      ...
      it('does not apply active styling when session is not active', () => {
        ...
        render(<AllSessionsView {...createDefaultProps({ sessions, activeSessionId: 'session-2' })} />);
  ...
  describe('GroupSection', () => {
    const createGroupedSessions = () => [
      createMockSession({ id: 's1', name: 'Session 1', groupId: 'group-1', groupName: 'Dev Group', groupEmoji: '🔧' }),
      createMockSession({ id: 's2', name: 'Session 2', groupId: 'group-1', groupName: 'Dev Group', groupEmoji: '🔧' }),
      createMockSession({ id: 's3', name: 'Session 3', groupId: 'group-2', groupName: 'Test Group', groupEmoji: '🧪' }),
    ];
    ...
    it('renders session count badge for each group', async () => {
      ...
      await waitFor(() => {
        ...
        const devGroupHeader = screen.getByRole('button', { name: /Dev Group group with 2 sessions/i });
        expect(devGroupHeader).toBeInTheDocument();

        // Test Group has 1 session
        const testGroupHeader = screen.getByRole('button', { name: /Test Group group with 1 sessions/i });
  ...
  describe('search functionality', () => {
    ...
    it('search is case insensitive', () => {
      const sessions = [
        createMockSession({ id: 's1', name: 'My Project' }),
      ];
    ...
    it('trims whitespace from search query', () => {
      const sessions = [
        createMockSession({ id: 's1', name: 'Session' }),
      ];
  ...
  describe('empty states', () => {
    it('shows empty state when no sessions exist', () => {
      ...
      expect(screen.getByText('Create a session in the desktop app to get started')).toBeInTheDocument();
    });
  ...
  describe('edge cases', () => {
    it('handles sessions with special characters in name', () => {
      const sessions = [
        createMockSession({ id: 's1', name: '<script>alert("xss")</script>' }),
      ];
    ...
    it('handles sessions with unicode in name', () => {
      const sessions = [
        createMockSession({ id: 's1', name: '日本語セッション 🎯' }),
      ];
  ...
  describe('accessibility', () => {
    ...
    it('session cards have proper aria-pressed state', () => {
      ...
      expect(screen.getByRole('button', { name: /Active.*active/i })).toHaveAttribute('aria-pressed', 'true');
      expect(screen.getByRole('button', { name: /Inactive session/i })).toHaveAttribute('aria-pressed', 'false');
    });

    it('group headers have aria-expanded state', async () => {
      const sessions = [
        createMockSession({ id: 's1', groupId: 'g1', groupName: 'Test Group' }),
      ];
  ...
  describe('integration scenarios', () => {
    ...
    it('handles dynamic session additions', async () => {
      ...
      const { rerender } = render(<AllSessionsView {...createDefaultProps({ sessions: initialSessions })} />);
