...
describe('TabSwitcherModal', () => {
  ...
  describe('pure helper functions', () => {
    ...
    describe('formatRelativeTime', () => {
      ...
      it('formats minutes ago', () => {
        const tab = createTestTab({
          logs: [{ id: '1', timestamp: Date.now() - 5 * 60 * 1000, source: 'stdout', text: 'test' }],
        });
      ...
      it('formats hours ago', () => {
        const tab = createTestTab({
          logs: [{ id: '1', timestamp: Date.now() - 3 * 60 * 60 * 1000, source: 'stdout', text: 'test' }],
        });
      ...
      it('formats days ago', () => {
        const tab = createTestTab({
          logs: [{ id: '1', timestamp: Date.now() - 2 * 24 * 60 * 60 * 1000, source: 'stdout', text: 'test' }],
        });
      ...
      it('formats as date for > 7 days ago', () => {
        const tab = createTestTab({
          logs: [{ id: '1', timestamp: Date.now() - 10 * 24 * 60 * 60 * 1000, source: 'stdout', text: 'test' }],
        });
  ...
  describe('rendering', () => {
    ...
    it('renders footer with navigation hints', () => {
      ...
      expect(screen.getByText('↑↓ navigate • Enter select • ⌘1-9 quick select')).toBeInTheDocument();
    });
  ...
  describe('keyboard navigation', () => {
    it('navigates down with ArrowDown', () => {
      const tabs = [
        createTestTab({ name: 'FirstItem' }),
        createTestTab({ name: 'SecondItem' }),
      ];
  ...
  describe('edge cases', () => {
    ...
    it('handles empty search with whitespace', () => {
      const tabs = [
        createTestTab({ name: 'Tab 1' }),
        createTestTab({ name: 'Tab 2' }),
      ];
  ...
  describe('accessibility', () => {
    ...
    it('uses semantic button elements for tabs', () => {
      const tabs = [
        createTestTab({ name: 'TabOne' }),
        createTestTab({ name: 'TabTwo' }),
      ];
      ...
      const tabButtons = screen.getAllByRole('button').filter(b =>
        b.textContent?.includes('TabOne') || b.textContent?.includes('TabTwo')
      );
