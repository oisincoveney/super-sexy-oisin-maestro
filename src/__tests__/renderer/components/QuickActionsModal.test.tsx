...
const createDefaultProps = (overrides: Partial<React.ComponentProps<typeof QuickActionsModal>> = {}) => ({
  theme: mockTheme,
...
describe('QuickActionsModal', () => {
  ...
  describe('Git actions', () => {
    ...
    it('handles Open Repository in Browser action', async () => {
      ...
      await waitFor(() => {
        ...
        expect(window.maestro.shell.openExternal).toHaveBeenCalledWith('https://github.com/test/repo');
        expect(props.setQuickActionOpen).toHaveBeenCalledWith(false);
      });
    });
  });
  ...
  describe('Keyboard navigation', () => {
    ...
    it('handles Cmd+0 for 10th item', () => {
      const props = createDefaultProps({
        sessions: Array(15).fill(null).map((_, i) => createMockSession({ id: `session-${i}`, name: `Session ${i}` })),
      });
      render(<QuickActionsModal {...props} />);
  ...
  describe('Scroll behavior', () => {
    it('scrolls selected item into view', async () => {
      ...
      const props = createDefaultProps({
        sessions: Array(20).fill(null).map((_, i) => createMockSession({ id: `session-${i}`, name: `Session ${i}` })),
      });
      render(<QuickActionsModal {...props} />);
