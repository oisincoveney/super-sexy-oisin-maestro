 ...
 */
function createMockClient(id: string, options?: {
  readyState?: number;
...
describe('BroadcastService', () => {
  ...
  describe('Concurrent Client Handling', () => {
    ...
    it('should handle mixed client states', () => {
      ...
      for (let i = 0; i < 25; i++) {
        clients.set(`closing-${i}`, createMockClient(`closing-${i}`, { readyState: WebSocket.CLOSING }));
      }
      for (let i = 0; i < 25; i++) {
        clients.set(`closed-${i}`, createMockClient(`closed-${i}`, { readyState: WebSocket.CLOSED }));
      }
